...
class VehicleIdDataModule(LightningDataModule):
    def __init__(
        self,
        data_dir: str = "data/",
        batch_size: int = 1,
        ...
        data = []
        with open(f"{to_absolute_path(data_dir)}/train_test_split/train_list.txt", "r") as f:
            for line in f:
                file_name, vid = line.strip().split(" ")
                data.append(
                    (
                        f"{to_absolute_path(data_dir)}/image/{file_name}.jpg",
                        vid,
                    )
                )
        ...
        data_test = []
        with open(f"{to_absolute_path(data_dir)}/train_test_split/{self.test_file}", "r") as f:
            for line in f:
                file_name, vid = line.strip().split(" ")
                data_test.append(
                    (
        ...
        self.df_test = pd.DataFrame(data_test, columns=["image_path", "vehicle_id"])
        self.all_ids_test = self.df["vehicle_id"].unique()
    ...
    def setup(self, stage: Optional[str] = None):
        if not self.data_train and not self.data_val and not self.data_test:
            if self.val_fold is not None:
                ...
                test_ids = self.all_ids_test
            else:

                train_ids, val_ids = train_test_split(self.all_ids, test_size=0.3, random_state=42)
                test_ids = self.all_ids_test

            self.data_train = VehicleIdDataset(
            ...
            self.data_val = VehicleIdDataset(
                self.df[self.df["vehicle_id"].isin(val_ids)],
                val_ids,
                test=False,
                transform=self.transforms,
            )
            self.data_test = VehicleIdDataset(
                self.df_test,
                test_ids,
                test=True,
                transform=self.transforms,
